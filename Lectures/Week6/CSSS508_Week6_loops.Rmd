...
date: "May 5, 2021<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
1. Set a number of iterations<sup>1</sup>
...
.footnote[[1] We could skip this, but it is useful if you want to make code flexible.]
...
.pull-right40[
.footnote[`levels = names(models)` sets the `Model` levels to be in the same order as our original vector!]
]

...
You can also add new columns and *drop old ones* using `transmute()`.<sup>1</sup>

.footnote[[1] I usually just use `mutate()` then `select()` for clarity.]
...
Note: If you want to see the `nycflights13` dataframes in the environment, you will need to load *each one* individually (`airlines`, `airports`, `flights`, `planes`, and `weather`) using `data(dataframe_name)` (e.g. `data(flights)`).

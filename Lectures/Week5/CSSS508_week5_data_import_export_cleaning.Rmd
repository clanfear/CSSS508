...
An improvement, but we don't want to keep the `r sum(is.na(billboard_2000$rank))` rows with missing ranks (i.e. observations for weeks since entering the Hot 100 that the song was no longer on the Hot 100).
...
```{r, cache=TRUE, echo=FALSE, dev="svg", fig.height=4}
...
```{r, cache=TRUE, echo=FALSE, dev="svg", fig.height=5}
jayz_bad_legend
```
...
```{r, cache=TRUE, echo=FALSE, fig.height = 5, dev="svg"}
jayz_good_legend
```

...
Remember when we tried find the mean for each variable in the `swiss` data? The best solution is to just use `colMeans()` without even thinking about pre-allocation or `for()` loops:
```{r}
colMeans(swiss)
```
...
Maybe you want to know more detailed quantile information than `summary()` gives you with interpretable names.

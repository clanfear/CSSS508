...
date: "Oct 17, 2018<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
Now we get more in the weeds of programming. Today is all about *types of data* in R.
...
In R, we call a set of values a **vector**. We can create vectors using the `c()` function ("c" for **c**ombine or **c**oncatenate).
...
.footnote[[1] R is perfectly happy with you including a calculation as an element!]
...
Like with dataframes, we can use `head()` and `tail()` to preview vectors:
...
str(my_list[[3]], list.len=9) # Displaying on first 9 elements
```
]
...
Dataframes are lists of equal-length vectors.

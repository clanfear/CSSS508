...
To get one row per precinct, we want to `spread()` key (`variable`) and value (`estimate`) columns into separate columns for each variable. Then we get `mutate()` `Any Black` into a proportion measure.

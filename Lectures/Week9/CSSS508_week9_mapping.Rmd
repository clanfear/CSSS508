...
Sometimes we want to plot these points over existing maps.
.small[
```{r, eval=FALSE}
ggplot(spd_raw, 
...
ggplot(spd_raw, aes(Longitude, Latitude)) + 
  geom_point() + 
  coord_fixed() +
  ggtitle("Seattle Police Incidents", subtitle="3/25/2018") +
  theme_classic()
```
]
...
.pull-right[
```{r qmplot, message = FALSE, echo=FALSE, fig.height = 5, fig.width=2.5, dev="svg"}
...
king_county_elections_2016 <- read_csv("./data/king_county_elections_2016.txt")

presidential_race <- king_county_elections_2016 %>%
  filter(Race=="US President & Vice President", str_detect(Precinct, "SEA ")) %>% 
  select(Precinct, CounterType, SumOfCount) %>%
  group_by(Precinct) %>%
  filter(CounterType %in% c("Donald J. Trump & Michael R. Pence", "Hillary Clinton & Tim Kaine", "Registered Voters", "Times Counted")) %>%
  mutate(CounterType=recode(CounterType, 
  ...
  mutate(P_Dem=Clinton/TotalVotes, P_Rep=Trump/TotalVotes, Turnout=TotalVotes/RegisteredVoters) %>%
  select(Precinct, P_Dem, P_Rep, Turnout) %>% filter(!is.na(P_Dem))

precincts_votes_sf <- precinct_shape %>% right_join(presidential_race)
```

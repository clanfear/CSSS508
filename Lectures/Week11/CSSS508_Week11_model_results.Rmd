...
Each row contains a predicted value for the outcome ("dependent") variable, plus confidence intervals. 

The argument `interval="prediction"` will give predicted intervals instead.
...
.small[
```{r, warning=FALSE, message=FALSE, dev="svg", fig.height=3.5}
lm(mpg~as.factor(am)+as.factor(cyl), data=mtcars) %>% 
  ggpredict(terms=c("am", "cyl")) %>% plot()
```
]
...
2. $\hat{\beta}$ is a vector of *random variables* whose distribution is estimated by the variance-covariance matrix.

--

Using this, we can do the following:

1. Extract the model matrix $X$, estimated coefficients ( $\hat{\beta}$ ), and *variance-covariance matrix* from our fitted model.
2. Make lots of random draws of $\hat{\beta}$ using the distribution described by the variance-covariance matrix.
3. Multiply *each* of these draws by the *counterfactual values* to get $\hat{y}$ values.
4. Take the mean plus the 2.5% and 97.% quantiles of these $\hat{y}$ values.
...
2. A meanour coefficient estimates—obtained via `coef()`)
3. A covariance matrix (obtained via `vcov()`)

.smallish[
```{r}
sim_params <- MASS::mvrnorm(n=10000, mu=coef(mod_arrest),
...
We want one row per estimate to plot, and all variables at their means *except* the ones we are varying.
...
.smallish[
```{r}
x_values <- colMeans(model.matrix(mod_arrest))
n_scen   <- (2*2*2*3)
...
Now we have 24 counterfactual scenarios (columns) with 10,000 estimates each (rows).
...
Then I add columns describing the scenarios to color, group, and facet over.
...
.small[
```{r}
estimated_pes %>% mutate_if(is.numeric, round, digits=3)
```
]

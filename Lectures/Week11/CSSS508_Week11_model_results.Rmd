...
ex_dat <- data.frame(x1 = rnorm(200, 1, 2), 
                     x2 = sample(c(1, 2, 3), 200, TRUE),
                     x3 = rnorm(200, 0, 3),
                     x4 = sample(c(1, 2))) %>%
  mutate(yn = x1*0.5 + x2*1.1 + x3*0.7 + x4-1.5  + rnorm(200, 0, 2)) %>% 
  mutate(yb = as.numeric(yn > mean(yn))) %>%
  mutate(x2 = factor(x2, labels=c("A", "B", "C")),
         x4 = factor(x4, labels=c("Yes", "No")))
```

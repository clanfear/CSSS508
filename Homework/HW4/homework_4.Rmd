...
# 3) Combine those two lists together using "c()" and assign them to "list_3"
list_3 <- NULL

# 4) Use "unlist()" to turn "list_3" into a vector and assign it to "vector_3"
vector_3 <- NULL

# 5) Use "as.list()" to convert "vector_3" into a list and assign it to "list_big"
list_big <- NULL

# 6) Now copy "list_3" as "list_4" and use "[[ ]]" to assign "list_3" as the last (fifth) element of "list_4".
list_4 <- NULL

# 7) Select the third element (that is, the sub-element) of the the fifth element of "list_4" and assign it 
...
# 5) Create a new data frame "df_3" using "data.frame()" that contains the following named columns (in order):
...
1) First, let's add `vec_cha` to df_1 to make a new data frame:
```{r data_frames_2}
df_cha <- data.frame(df_1, vec_cha)
```
Use `lapply(df_cha, class)` to get a list that indicates the class of each column
in `df_1`. Do you see anything unusual about `df_cha$vec_cha`, say if we get `class(vec_cha)`? Get help on data frames with
